...
import {failure, pkexec, run, commandLine} from './exec.js';
...
/**
 * A size as snapper printed it.
 *
 * The used-space column is written for people to read, so depending on the
 * version it comes back as plain bytes or as something like 1.50 MiB, and as
 * nothing at all when btrfs has no quota group to ask.
 *
 * @param {string} cell - one cell of snapper's output
 * @returns {number|null} bytes, or null where there was no figure in it
 */
function bytes(cell) {
    // A figure with a comma in it - which is how 1,50 GiB is written where the
    // comma is the decimal separator - comes back from --csvout inside quotes,
    // and they are not part of the number. In a quoted cell a doubled quote
    // stands for one.
    ...
    const [, figure, unit] = /^([\d.,]+)\s*([KMGTPE]?)i?B$/i.exec(text) ?? [];
    if (!figure)
        return null;

    const scale = unit ? 'KMGTPE'.indexOf(unit.toUpperCase()) + 1 : 0;
    return Number(figure.replace(',', '.')) * Math.pow(1024, scale);
}

/**
 * What one config's snapshots are holding on to.
 *
 * The figure is per snapshot the space that snapshot alone is keeping alive -
 * what would come back if it went - so the total is what the whole set costs
 * over and above the subvolume they were taken of. btrfs can only answer it
 * where quotas are set up, and snapper leaves the column empty rather than
 * complaining where they are not.
 *
 * Asking is not free: snapper rescans the quota groups first, which on a
 * filesystem with a lot of snapshots on it takes its time.
 *
 * @param {string} config - config name
 * @param {Gio.Cancellable} [cancellable] - what says the answer is no longer
 *   wanted, which ends the rescan too
 * @returns {Promise<{count: number|null, bytes: number|null, said: string|null}>}
 *   how many snapshots it has and what they take between them, or a count of
 *   null and what snapper said when it would not answer at all
 */
export async function snapshotSpace(config, cancellable = null) {
    const result = await run(['snapper', '-c', config, '--csvout',
        'list', '--columns', 'number,used-space'], cancellable);
    // A config this account has not been let into refuses to be listed, which
    // is a different answer from a config with nothing in it and must not be
    // shown as one.
    if (result.status !== 0)
        return {count: null, bytes: null, said: failure(result)};
    ...
    for (const line of stdout.split('\n').slice(1)) {
        ...
        // Snapshot 0 is the subvolume as it is now rather than a snapshot of
        // it, and it holds everything, so counting it would answer a different
        // question than the one being asked.
        if (number.trim() === '' || number.trim() === '0')
            continue;

        count += 1;
        const own = bytes(rest.join(','));
        if (own !== null)
            total = (total ?? 0) + own;
    }
    return {count, bytes: total, said: null};
}

/**
 * @param {string} config - the config whose subvolume gets a quota group
 * @returns {string[]} the command that would do it
 */
export function setupQuotaArgv(config) {
    return ['snapper', '-c', config, 'setup-quota'];
}
 ...
 * @returns {Promise<{status: number, stdout: string, stderr: string}>} how it went
 */
export function setupQuota(config) {
    return pkexec(setupQuotaArgv(config));
}
