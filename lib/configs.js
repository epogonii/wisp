...
import {pkexec, run, commandLine} from './exec.js';
 ...
 * @returns {Promise<{count: number, bytes: number|null}>} how many snapshots
 *   it has, and what they take between them where that can be worked out
 */
export async function snapshotSpace(config) {
    const {status, stdout} = await run(['snapper', '-c', config, '--csvout',
