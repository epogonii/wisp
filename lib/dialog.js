 ...
 *
 * A caller with something to say about how long it is taking passes a function
 * for it, and gets a line over the bar that is asked once a second. It sits on
 * the bar rather than under the note because that is what it is about: the two
 * read as one thing, and the note stays a paragraph rather than something with
 * a number ticking away at the end of it. The line belongs to the bar and goes
 * when the bar goes, which is how the second it is counting can never be
 * counted into a dialog that has been closed.
 */
...
class WaitBar extends St.Widget {
    _init(tick = null) {
        super._init({
            ...
            x_expand: true,
            // Whatever the stripe's width works out to, it cannot be painted
            // outside the bar it belongs to.
            clip_to_allocation: true,
        });

        // No layout manager and no alignment: where the stripe goes is set by
        // hand below. Asking a layout to align it to the start of the bar left
        // it sitting in the middle instead, which is where the slide began and
        // half a span past the far end is where it ended.
        ...
        this._tall = -1;
        this._pending = 0;
        this.connect('notify::allocation', () => this._queue());

        this._tick = tick;
        this._aside = null;
        this._ticking = 0;
        if (tick) {
            this._aside = new St.Label({style_class: 'wisp-aside'});
            wrap(this._aside);
            // The dialog puts it over the bar rather than inside it, since a
            // bar four pixels tall has no room for a line of text, so it is
            // taken away by hand.
        ...
        this.connect('destroy', () => {
            ...
            this._pending = 0;
            if (this._ticking)
                GLib.source_remove(this._ticking);
            this._ticking = 0;
    ...
    /**
     * The line the bar keeps up to date, for the dialog to place.
     *
     * @returns {St.Label|null} the line, or nothing if there is nothing to say
     */
    get aside() {
        return this._aside;
    }
    ...
    /**
     * Asks what to say now, and says it. An empty answer means the line is not
     * worth the room yet: a wait that is over in a second should not flash a
     * count on the way past.
     */
    _say() {
        const text = this._tick();
        this._aside.text = text;
        this._aside.visible = text !== '';
    }

    /**
     * Puts off the measuring until the layout is over.
     *
     * A width is a layout property, and this signal arrives in the middle of a
     * layout, where asking for another one is dropped on the floor. Setting the
     * width here works the first time only, when the stripe has yet to be laid
     * out at all: after that the stripe keeps the width it was given for a
     * narrower bar while the distance it travels grows with the wider one, and
     * it walks out of the far end of the dialog.
     */
    _queue() {
        if (this._pending)
            return;
    ...
    /**
     * Sizes the stripe against the bar and starts it moving.
     *
     * How wide the bar is, is up to the dialog, and it is not a width anybody
     * set: it arrives with the allocation, which is the one thing that says so.
     * Waiting for notify::width is what left the stripe standing still, because
     * a width nobody set is never notified; the stripe was sized once, against
     * a bar that had no width yet, and a stripe of no width has nothing to show
     * and nowhere to slide.
     *
     * The stripe is given a size and a corner of its own rather than a size and
     * an alignment for a layout to work the corner out from. Which is why this
     * waits for an idle rather than running where the allocation is announced:
     * a size is a layout property, and setting one in the middle of a layout is
     * dropped on the floor; see _queue.
     */
    _measure() {
        const track = Math.floor(this.allocation.get_width());
        const tall = Math.floor(this.allocation.get_height());
        if (track <= 0 || tall <= 0)
            return;

        const stripe = Math.max(WAIT_MIN, Math.round(track * WAIT_STRIPE));
        const span = Math.max(0, track - stripe);

        // Being laid out again at the same size must not start the slide over,
        // or the stripe stands at one end for as long as the wait lasts.
        if (span === this._span && tall === this._tall)
            return;

        this._span = span;
        this._tall = tall;
        this._stripe.set_size(stripe, tall);
        this._stripe.set_position(0, 0);
        this._stripe.remove_all_transitions();
...
// Every dialog on the screen, so that switching the extension off can take
// them with it. A dialog holds the keyboard and the pointer while it is open,
// and one left behind by an extension that is no longer running would hold
// them against nothing.
const onScreen = new Set();

/**
 * Closes whatever is still open. Called when the extension is switched off.
 */
export function closeAll() {
    for (const dialog of [...onScreen]) {
        // close() is what hands the grab back, and it is synchronous. The
        // destroy after it is for the fade: the actor would otherwise be a
        // quarter of a second in going, and by then there is nothing left of
        // the extension that put it there.
        dialog.close();
        dialog.destroy();
    }
}
...
class WispDialog extends ModalDialog.ModalDialog {
     ...
     * @param {Function} [tick] - asked once a second for a line over the bar
     *   about how long it is taking; it returns the words, or '' for no line
     */
    wait(text, tick = null) {
        if (this._gone)
            return;
        ...
        this.note(text);
        if (!this._wait) {
            this._wait = new WaitBar(tick);
            this.contentLayout.insert_child_below(this._wait, this._note);
            if (this._wait.aside)
                this.contentLayout.insert_child_below(this._wait.aside, this._wait);
        }
    }

    /**
     * Takes the bar away, and the line over it with it. The note stays: what
     * replaces it is the answer.
     */
    stopWait() {
