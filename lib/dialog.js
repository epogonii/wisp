 ...
 * for it, and gets a line under the note that is asked once a second. The line
 * belongs to the bar and goes when the bar goes, which is how the second it is
 * counting can never be counted into a dialog that has been closed.
 */
...
class WaitBar extends St.Widget {
    _init(tick = null) {
        ...
        if (tick) {
            ...
            wrap(this._aside);
            // The dialog puts it where it reads best, which is under the note
            // and not inside the bar, so it can be taken away by hand.
...
class WispDialog extends ModalDialog.ModalDialog {
     ...
     * @param {Function} [tick] - asked once a second for a line about how long
     *   it is taking; it returns the words, or '' for no line at all
     */
    wait(text, tick = null) {
        if (this._gone)
            return;
        ...
        if (!this._wait) {
            ...
            if (this._wait.aside)
                this.contentLayout.add_child(this._wait.aside);
        }
    }

    /**
     * Takes the bar away, and the line under the note with it. The note stays:
     * what replaces it is the answer.
     */
    stopWait() {
