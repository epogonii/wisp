...
export const Snapper = GObject.registerClass({
    ...
    /**
     * The names of the configs snapper knows about, in the order it keeps
     * them. ListConfigs is the one call snapperd answers for anybody, so this
     * works before any of the permissions are in place.
     *
     * @returns {Promise<string[]>} config names, or nothing if snapperd is away
     */
    async listConfigs() {
        const configs = await this.listConfigDetails();
     ...
     *
     * @param {string} config - config name, as listConfigs returns them
    ...
    /**
     * The snapshot the system is running from, for the configs where that
     * means anything. Nothing else in the menu should offer to delete it.
     *
     * @param {string} config - config name
     * @returns {Promise<number|null>} its number, or null if there is not one
     */
    async activeSnapshot(config) {
        const [found, number] = await this._proxy.GetActiveSnapshotAsync(config);
