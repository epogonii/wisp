...
export const Snapper = GObject.registerClass({
    ...
    async usedSpace(config, number) {
        try {
            ...
            // Two names mean the figure is not there to be had, and both of
            // them are quotas being off: CalculateUsedSpace answers
            // error.quota, and GetUsedSpace - where the rescan did go through
            // - error.something. Everything else is a real fault and belongs
            // in front of whoever asked: a config that is gone, a snapshot
            // that is gone, a call that was refused.
            const name = Gio.DBusError.get_remote_error(error);
            if (name === 'error.quota' || name === 'error.something')
                return null;
            throw error;
        }
    }
     ...
     *
     * This is called again every time the list of configs is read, which is
     * every menu open and every change the watching itself reports. The
     * directories are almost always the same ones, so the monitors already
     * running are kept and only the difference is acted on: without that, a
     * snapshot appearing would cancel and remake every monitor, including the
     * one that had just reported it.
     *
     * @param {{subvolume: string}[]} configs - what to watch, as listConfigDetails
     *   gives them
     */
    watch(configs) {
        const paths = configs.map(({subvolume}) => subvolume === '/'
            ? '/.snapshots'
            : `${subvolume}/.snapshots`);
        ...
        for (const path of paths) {
            ...
            try {
                ...
                // Nothing is remembered about the failure, so being given
                // access starts the watching on the next read - the list of
                // directories does not change when the ACL does.
