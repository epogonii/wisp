...
class AuthDialog extends WispDialog {
    _init({title, description, argv, confirmLabel, danger = false, onDone = null,
        ...
        // Where the command reads a file the caller wrote for it - a list of
        // paths too long to be arguments - that file has to be there for as
        // long as the command may still run, and it may still run while the
        // dialog is gone: pkexec is answered on its own time and being
        // disabled from under it does not stop it. So the caller is told to be
        // rid of it once pkexec has answered, or once the dialog has gone
        // without pkexec having been asked anything.
    ...
    async _run() {
        ...
        try {
            ...
            // pkexec was answered after the dialog went away: Cancel while it
            // was pending, or the extension being disabled from under it. The
            // answer is nobody's now.
            if (this._done || !this.alive)
                return;
            ...
            this._running = false;
            // A dialog still up is a dialog offering Try again, and the next
            // try needs whatever this one was reading.
            if (this._done || !this.alive)
                this._clean();
        }
    }

    /** Lets go of what was put aside for the command, once and once only. */
    _clean() {
    ...
    _finish(result) {
        // Whoever gets here first decides. Cancel and a finished pkexec race
        // each other, and the caller is told once either way.
        if (this._done)
            return;
        this._done = true;
