 ...
 *
 * systemd asks polkit for each step. Its policy has the unit file action
 * imply the reload and the start, so one answer covers all three where the
 * policy has not been changed. A refusal comes back as a rejected promise.
 *
 * @param {string} unit - a unit name
...
export async function setEnabled(unit, on) {
    ...
    // The links were changed on disk. systemctl reloads the manager after
    // enable and disable so that it sees them, and so does this.
    await call('Reload', null, null);

    await call(on ? 'StartUnit' : 'StopUnit',
