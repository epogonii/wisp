...
export function check(snapper, configs) {
    if (!GLib.find_program_in_path('snapper')) {
        return {
            problem: MISSING,
            message: _('Wisp shows the snapshots snapper takes, and snapper is not installed. Install it as root:'),
    ...
    if (configs.length === 0) {
        return {
            problem: NO_CONFIGS,
            message: _('snapper has nothing set up yet. One config per subvolume is what it takes snapshots of. Run this as root to make the first one:'),
