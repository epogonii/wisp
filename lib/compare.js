        ...
        try {
            ...
            if (file.created) {
                if (this._to === CURRENT) {
                    ...
                    said = _('Opened the file as it is now');
                } else {
            ...
            const uri = Gio.File.new_for_path(root + file.path).get_uri();
            Gio.AppInfo.launch_default_for_uri(uri, null);
            Toast.announce(said);
        } catch (error) {
            if (!this.alive)
                return;
            // Working out what would open it means reading the file, so a
            // snapshot directory carrying no ACL refuses here, before
            // anything opens.
            this.note(error.matches?.(Gio.IOErrorEnum, Gio.IOErrorEnum.PERMISSION_DENIED)
                ? _('Snapshot files are not readable by this account. SYNC_ACL=yes on the config is what opens them up.')
                : error.message, true);
        }
    }
