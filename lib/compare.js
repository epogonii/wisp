 ...
 *
 * Putting it back is snapper's own undochange, which the user runs as root. It
 * is not a copy of the file: undochange restores what was deleted, deletes what
 * was created, and carries permissions, ownership and extended attributes
 * across, none of which a copy from the snapshot directory would do.
 */
...
class CompareDialog extends WispDialog {
    ...
    /**
     * Shows the undochange command for the chosen paths.
     *
     * The list goes in a file rather than on the command line: a few hundred
     * paths is an ordinary selection and an argument list has a limit. The
     * command is run later, by hand, so the file stays in the cache directory.
     * Each restore gets a file of its own: a command copied earlier and run
     * after another restore was chosen must still name its own paths.
     */
    async _restore_selected() {
        ...
        const dir = GLib.build_filenamev([GLib.get_user_cache_dir(), 'wisp']);
        GLib.mkdir_with_parents(dir, 0o700);
        const list = GLib.build_filenamev([dir,
            `restore-${GLib.uuid_string_random()}.txt`]);
        GLib.file_set_contents(list, `${paths.join('\n')}\n`);
        ...
        new CommandDialog({
            ...
            argv: ['snapper', '-c', this._config, 'undochange', '-i', list,
                `${this._from}..${this._to}`],
            danger: true,
        }).open();
    }
