...
class DetailsDialog extends WispDialog {
    ...
    _delete() {
        ...
        new ConfirmDialog({
            title: ngettext('Delete snapshot %d?', 'Delete snapshots %d and %d?', n)
                .format(...numbers),
            ...
            onConfirm: () => {
                this._snapper.delete(this._config, numbers)
                    ...
                    .catch(error => {
                        ...
                        new AuthDialog({
                            title: ngettext('Delete snapshot %d?',
                                'Delete snapshots %d and %d?', n).format(...numbers),
...
class CreateDialog extends WispDialog {
    ...
    _create() {
        ...
        this._snapper.create(this._config, description)
            .then(number => {
                // A pill under the panel rather than a notification: this is
                // the answer to something asked a second ago from up there, and
                // it is not worth keeping in the message list afterwards.
                Toast.show(_('Snapshot %d taken').format(number));
                if (this.alive)
                    this.close();
            })
            .catch(error => {
