...
function denied(config) {
    return _('This account is not allowed to change %s. The menu shows the command that gives it access.').format(config);
}
...
class DetailsDialog extends WispDialog {
    ...
    _rollback() {
        ...
        // grow one without moving everything under it.
        rootSubvolumePinned()
            .then(pinned => {
                if (!this.alive)
                    return;
